...
function authenticated() {
  cy.intercept({method:'get',url:'http://localhost:8080/users/me'}).as('me')
  cy.visit('http://localhost:3000/auth/sign-in')
  cy.get('input#cpf').type('52106796838')
...
describe('User',()=>{
  ...
  it('should be able create a opportunity', () => {
    ...
    cy.intercept({ method: 'post', url: 'http://localhost:8080/opportunities' }).as('opportunity')
    cy.get('[title="Publique sua oportunidade"]').click()

    const startDate = faker.date.soon({days:1,refDate:new Date().setDate(1)}).toJSON().split('T')[0]
    const endDate = faker.date.future({years:8}).toJSON().split('T')[0]
    ...
    cy.get('textarea#description').type('aqui vem um texto muito legal para eu colocar na descrição eba q legal')
